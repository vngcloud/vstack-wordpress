...
class HTTP2ServerPush
{
    ...
    public static function initHooks()
    {
        ...
        add_action('wp_head', array('\VSTACK\WordPress\HTTP2ServerPush', 'http2ResourceHints'), 99, 1);
        ...
        if (class_exists('autoptimizeMain')) {
            add_filter('autoptimize_filter_cache_getname', array('\VSTACK\WordPress\HTTP2ServerPush', 'http2LinkPreloadHeader'), 99, 1);
            if (!$autoptimize_js_enabled) {
                add_filter('script_loader_src', array('\VSTACK\WordPress\HTTP2ServerPush', 'http2LinkPreloadHeader'), 99, 1);
            }
            if (!$autoptimize_css_enabled) {
                add_filter('style_loader_src', array('\VSTACK\WordPress\HTTP2ServerPush', 'http2LinkPreloadHeader'), 99, 1);
            }
        } else {
    ...
    public static function http2LinkPreloadHeader($src)
    {
        if (strpos($src, home_url()) !== false) {
            ...
            if (!empty($preload_src)) {
                ...
                if (defined('VSTACK_HTTP2_SERVER_PUSH_HEADER_SIZE')) {
                    $maxHeaderSize = absint(VSTACK_HTTP2_SERVER_PUSH_HEADER_SIZE);
                };

                if ($headerSize > $maxHeaderSize) {
                    if (defined('VSTACK_HTTP2_SERVER_PUSH_LOG') && VSTACK_HTTP2_SERVER_PUSH_LOG) {
