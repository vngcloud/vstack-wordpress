...
class Hooks
{
    ...
    const VSTACK_JSON = 'VSTACK_JSON';
    const WP_AJAX_ACTION = 'vstack';

    public function __construct()
    {
        $this->config = new Integration\DefaultConfig(file_get_contents(VSTACK_PLUGIN_DIR.'config.json', true));
    ...
    /**
     * @param \VSTACK\API\APIInterface $api
     */
    ...
    public function cloudflareConfigPage()
    {
        if (function_exists('add_options_page')) {
            add_options_page(__('vStack Configuration'), __('vStack'), 'manage_options', 'vstack', array($this, 'cloudflareIndexPage'));
        }
    }

    public function cloudflareIndexPage()
    {
        include VSTACK_PLUGIN_DIR.'index.php';
    }

    public function pluginActionLinks($links)
    {
        $links[] = '<a href="'.get_admin_url(null, 'options-general.php?page=vstack').'">Settings</a>';
    ...
    {
        if (version_compare($GLOBALS['wp_version'], VSTACK_MIN_WP_VERSION, '<')) {
            deactivate_plugins(basename(VSTACK_PLUGIN_DIR));
            wp_die('<p><strong>Cloudflare</strong> plugin requires WordPress version '.VSTACK_MIN_WP_VERSION.' or greater.</p>', 'Plugin Activation Error', array('response' => 200, 'back_link' => true));
        }
    ...
    {
        $cacheSettingObject = $this->dataStore->getPluginSetting(\VSTACK\API\Plugin::SETTING_PLUGIN_SPECIFIC_CACHE);

        if (! $cacheSettingObject) {
            return false;
        }

        $cacheSettingValue = $cacheSettingObject[\VSTACK\API\Plugin::SETTING_VALUE_KEY];
    ...
    {
        $cacheSettingObject = $this->dataStore->getPluginSetting(\VSTACK\API\Plugin::SETTING_AUTOMATIC_PLATFORM_OPTIMIZATION);
     ...
     * is coming from the cloudflare proxy.  We store it in a global so \VSTACK\WordPress\Proxy
    ...
    public function getCloudflareRequestJSON()
    {
        if (isset($_GET['action']) && $_GET['action'] === self::WP_AJAX_ACTION) {
            $GLOBALS[self::VSTACK_JSON] = file_get_contents('php://input');
        }
    }
