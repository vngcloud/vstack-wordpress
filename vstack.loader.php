...
$cloudflareHooks = new \VSTACK\WordPress\Hooks();

add_action('plugins_loaded', array($cloudflareHooks, 'getCloudflareRequestJSON'));

// Enable HTTP2 Server Push
if (defined('VSTACK_HTTP2_SERVER_PUSH_ACTIVE') && VSTACK_HTTP2_SERVER_PUSH_ACTIVE && !is_admin()) {
...
if (is_admin()) {
    ...
    add_action('wp_ajax_vstack', array($cloudflareHooks, 'initProxy'));
    ...
    register_deactivation_hook(VSTACK_PLUGIN_DIR.'vstack.php', array($cloudflareHooks, 'deactivate'));
}
